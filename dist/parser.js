 ...
 */
export function parseHJX(source, filename = "<input>") {
    ...
    // Skip blank/comment
    const isSkippable = (s) => /^\s*$/.test(s) || /^\s*\/\//.test(s);
    ...
    while (i < lines.length) {
        ...
        if (trimmed === "state:") {
            ...
            while (i < lines.length) {
                ...
                if (!m)
                    err("Invalid state line. Expected: name = value", i);
                const key = m[1];
                const raw = m[2].trim();
        ...
        if (trimmed === "computed:") {
            ...
            while (i < lines.length) {
                ...
                const m = t.match(/^([A-Za-z_][A-Za-z0-9_]*)\s*=\s*(.+)$/);
                if (!m)
                    err("Invalid computed line. Expected: name = expression", i);
                const key = m[1];
                const raw = m[2].trim();
        ...
        if (trimmed === "imports:") {
            ...
            while (i < lines.length) {
                ...
                if (!m)
                    err("Invalid import line. Expected: Alias from \"path\"", i);
                const key = m[1];
                const raw = m[2].trim();
        ...
        if (trimmed === "api:") {
            ...
            while (i < lines.length) {
                ...
                if (!apiMatch)
                    err("Invalid API line. Expected: METHOD /path -> handlerName", i);
                const endpoint = {
                    method: apiMatch[1],
                    path: apiMatch[2],
                ...
                while (i < lines.length) {
                    ...
                    if ((st === "query:") || (st === "params:") || (st === "body:")) {
                        ...
                        while (i < lines.length) {
                            ...
                            if (!vm)
                                err(`Invalid ${sectionType} field`, i);
                        ...
                        while (i < lines.length) {
                            ...
                            if (rm[1] === "type") {
                                ...
                                response.item = rawVal;
                            else if (rm[1] === "schema")
                                response.schema = rawVal;
                            i++;
                        }
        ...
        if (trimmed === "breakpoints:") {
            ...
            while (i < lines.length) {
                ...
                const m = t.match(/^([A-Za-z_][A-Za-z0-9_]*)\s*=\s*(.+)$/);
                if (!m)
                    err("Invalid breakpoint line. Expected: name = value", i);
        ...
        if (trimmed === "handlers:") {
            ...
            while (i < lines.length) {
                ...
                if (!hm)
                    err("Invalid handler header. Expected: name:", i);
                const name = hm[1];
                i++;
                const body = [];
                while (i < lines.length) {
                    ...
                    if (ind <= 2)
                        break; // back to handlers block level
                    body.push(bl.trim());
                    i++;
                }
...
function parseStateValue(raw, onError) {
    // string
...
function parseLayout(lines, getIndex, setIndex, filename) {
    ...
    function parseParenContent(node, content) {
