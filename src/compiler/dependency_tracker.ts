import { HJXAst, HJXNode } from "../types";
import { 
  DependencyGraph, 
  ElementExpression, 
  AnalysisContext,
...
export class DependencyTracker {
  ...
  private dependencyCache = new Map<string, {deps: Set<string>, timestamp: number}>();
   ...
   */
  analyze(ast: HJXAst): AnalysisResult {
   ...
   */
  private generateElementId(path: number[]): string {
   ...
   */
  private analyzeText(text: string | null, elementId: string, context: AnalysisContext) {
    if (!text) return;
    
    const deps = this.extractDependencies(text, context);
    if (deps.size === 0) return;
    
    const expression: ElementExpression = {
      type: 'text',
      expression: text,
      dependencies: deps
    };
   ...
   */
  private analyzeAttributes(attrs: Record<string, string> | undefined, elementId: string, context: AnalysisContext) {
    if (!attrs) return;
   ...
   */
  private analyzeCondition(condition: string | undefined, elementId: string, context: AnalysisContext) {
    if (!condition) return;
   ...
   */
  private analyzeLoop(iterator: { item: string; list: string } | undefined, elementId: string, context: AnalysisContext) {
    if (!iterator) return;
   ...
   */
  private analyzeEvents(events: Record<string, string> | undefined, elementId: string, context: AnalysisContext) {
    if (!events) return;
   ...
   */
  private analyzeBindings(bind: { prop: string; state: string } | null, elementId: string, context: AnalysisContext) {
    if (!bind) return;
   ...
   */
  private extractDependencies(expr: string, context: AnalysisContext): Set<string> {
    // Check cache first
   ...
   */
  private extractDependenciesImpl(expr: string, context: AnalysisContext): Set<string> {
 ...
 */
private resolveFullDependencyChain(directDeps: Set<string>): Set<string> {
   ...
   */
  private processDependencyToken(token: string, context: AnalysisContext, deps: Set<string>): void {
    const rootKey = token.split('.')[0];
    
    // Skip if not a valid state key
    if (!context.stateKeys.has(rootKey)) return;
    
    // Skip reserved words
    if (this.isReservedWord(rootKey)) return;
    
    // Skip current loop item
    if (rootKey === context.currentLoopItem) return;
   ...
   */
  private mapDepsToElement(deps: Set<string>, elementId: string) {
    if (this.batchMode) {
   ...
   */
  private addElementExpression(elementId: string, expression: ElementExpression) {
   ...
   * Analyze computed values
   */
  private analyzeComputed(computed: Record<string, string>, stateKeys: Set<string>) {
  ...
  private checkCircularDependencies() {
    ...
    const dfs = (key: string, stack: Set<string>) => {
      if (stack.has(key)) {
   ...
   */
  private analyzeHandlers(handlers: Record<string, any>, stateKeys: Set<string>) {
    for (const [handlerName, handler] of Object.entries(handlers)) {
      const modified = new Set<string>();
      
      if (handler.body && Array.isArray(handler.body)) {
        handler.body.forEach((line: string) => {
   ...
   */
  private checkForOrphanedElements() {
   ...
   */
  private checkForInvalidReferences() {
   ...
   */
  public getAffectedElements(stateKey: string): Set<string> {
  const elements = new Set<string>();
  const processedComputed = new Set<string>();
  
  // Helper to find elements through computed chains recursively
  const collectFromComputed = (computedKey: string) => {
    if (processedComputed.has(computedKey)) return;
    ...
    return {
      ...
      edges: {
        stateToElement: Array.from(this.graph.stateToElements.entries())
          .flatMap(([state, elements]) => 
            Array.from(elements).map(el => ({ from: state, to: el }))
          ),
        stateToComputed: Array.from(this.graph.stateToComputed.entries())
          .flatMap(([state, computed]) => 
            Array.from(computed).map(comp => ({ from: state, to: comp }))
          ),
