...
Use the `api:` block to define REST endpoints:
...
## Using API in Handlers

Call API endpoints from event handlers using `fetch`:
...
Generate Express.js routes and handlers:
...
Define request/response schemas for type safety:
...
Define API endpoints using natural language:
...
- Explore [Flow-State Engine](./flow-state)
